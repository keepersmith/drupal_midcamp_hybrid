...
final class Hooks {
  ...
  public static function uninstallProfile(): void {
    ...
    // to redirect into Drupal with a stale container. Also, ExtensionList
    // caches profile information and even worse, stores it in state which
    // also needs to be cleaned up just in case all that was set in a parallel
    // request while the profile was being installed. And these are just the
    // bugs already encountered in the wild, let's stem the unknown ones at
    // the root: nuke it from orbit, it's the only way to make sure.
